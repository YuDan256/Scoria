...
static TokenKind identifier_type(Lexer* lexer) {
    ...
    if (len == 3 && memcmp(lexer->start, "red", 3) == 0) return TK_KW_REDDE;

    // 【古典宗卷长体映射网络】
    switch (c) {
    case 'a':
        if (len == 5) return check_keyword(lexer, 1, 4, "ctio", TK_KW_ACTIO);
    ...
    case 's':
        ...
        break;
    case 't': return check_keyword(lexer, 1, 3, "ene", TK_KW_TENE);
    case 'v':
...
static Token number(Lexer* lexer) {
    if (lexer->start[0] == '0') {
        ...
        if (prefix == 'r' || prefix == 'R') {
            advance(lexer);
            while (strchr("IVXLCDMivxlcdm", peek(lexer))) advance(lexer);
            return make_token(lexer, TK_INT_CONST);
        }
    }
